...
const userNavItems = [
  ...
  { to: '/dashboard/change-password', label: 'Change Password', icon: '🔐' },
  { to: '/dashboard/settings', label: 'Account Settings', icon: '⚙️' },
        ...
        <nav className="space-y-2 px-4 py-6">
          {userNavItems.map((item) => (
            <Link
              key={item.to}
              to={item.to}
              className="flex items-center gap-3 rounded-xl px-4 py-3 text-slate-300 transition hover:bg-violet-500/10 hover:text-white"
            >
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="hidden items-center md:flex">
              <input
                ...
                className="rounded-xl border border-white/10 bg-slate-900/80 px-4 py-2.5 text-sm text-slate-100 placeholder:text-slate-400 outline-none ring-0 transition focus:border-violet-400"
              />
            </div>
          </div>
          <div className="flex items-center gap-4">
