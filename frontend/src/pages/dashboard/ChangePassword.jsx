...
export default function ChangePasswordPage() {
  ...
  const handleSubmit = async e => {
    ...
    if (!isAdmin) {
      setStatus({
        type: 'error',
        message: 'Password resets must be approved by an administrator.'
      });
      return;
    }
  ...
  return <div className="rounded-2xl bg-white p-6 shadow-sm">
      ...
      {!isAdmin ? <div className="rounded-xl border border-amber-200 bg-amber-50 p-5 text-amber-900">
          ...
          <PasswordInput label="Current password" name="currentPassword" value={form.currentPassword} onChange={handleChange} error={errors.currentPassword} placeholder="Current password" />

          <div>
            <PasswordInput label="New password" name="newPassword" value={form.newPassword} onChange={handleChange} error={errors.newPassword} placeholder="New password" />

            {form.newPassword && <div className="mt-3 space-y-2">
                <div className="h-2 overflow-hidden rounded-full bg-slate-200">
                  <div className={`h-full rounded-full ${strengthColors[strength.label] || 'bg-slate-300'}`} style={{
          ...
          <PasswordInput label="Confirm password" name="confirmPassword" value={form.confirmPassword} onChange={handleChange} error={errors.confirmPassword} placeholder="Confirm password" />
