...
export default function Register() {
  ...
  return <div className="w-full max-w-md">
      ...
      <form onSubmit={handleSubmit} className="mt-8 space-y-5">
        <Input label="Full name" type="text" name="fullName" placeholder="Jane Doe" value={form.fullName} onChange={handleChange} error={errors.fullName} />
        ...
        <Input label="Phone number" type="tel" name="phone" placeholder="+1 555 123 4567" value={form.phone} onChange={handleChange} error={errors.phone} />

        <div>
          <PasswordInput label="Password" name="password" placeholder="••••••••" value={form.password} onChange={handleChange} error={errors.password} />
          {form.password && <p className="mt-2 text-xs text-slate-500">
              Password strength: <span className="font-semibold text-slate-700">{passwordStrength.label}</span>
            </p>}
        </div>

        <PasswordInput label="Confirm password" name="confirmPassword" placeholder="••••••••" value={form.confirmPassword} onChange={handleChange} error={errors.confirmPassword} />

        {submitMessage && messageType === 'success' && <SuccessMessage message={submitMessage} />}

        {submitMessage && messageType === 'error' && <ErrorMessage message={submitMessage} />}
